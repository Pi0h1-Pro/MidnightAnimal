...
draw_set_font(fntTiny); draw_set_halign(fa_left);
draw_set_colour(c_ltgray);draw_text(64,16,string_hash_to_newline("///DEBUG LEVEL SELECT///"));
for (var i = ds_list_size(levelList) - 1; i > -1; i--;){
    if select=i draw_set_colour(global.rubedo) else draw_set_colour(global.albedo);
