if scrIsDodging(sprite_index) {
    if !place_meeting(x, y, objBullet) and !place_meeting(x, y, objWeaponThrow) {
        if image_index < 15 image_index += 0.25
        else {
            if object_index = objEnemyDodger sprite_index = sprEDodgerWalk
            legindex = 0
        }
    } else {
...
if path_index > 0 {
    ...
    if killed = 0 image_speed = speed * 0.1
}
scrMoveSolidOff()
