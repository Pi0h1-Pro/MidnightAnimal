if (sprite_index == sprPReloadSniper || sprite_index == sprPReloadSuppressedMG || sprite_index == sprPReloadSilencedShotgun)
{
	if (sprite_index == sprPReloadSuppressedMG)
	{
		...
		{
			image_index += 0.25;
			if (image_index * 10 == 70)
				sound_play(sndReload4);
		}
		else
		{
			if (objPlayer.mags > 0)
	...
	{
		if (objPlayer.mags > 0 && ammo < global.maxammo)
		{
