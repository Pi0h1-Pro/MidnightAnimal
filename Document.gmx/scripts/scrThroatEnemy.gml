if sprite_index = sprPWalkBag or sprite_index = sprPAttackBag or global.maskindex = 2 nothing = 1
else {
    global.test = 0
    if place_meeting(x + lengthdir_x(12, dir), y + lengthdir_y(12, dir), objEnemy) {
        with objEnemy {
            ...
            or scrIsPolice(object_index) 
            //or scrIsGang(object_index)
            or global.test > 0 nothing = 1
            else {
                if place_meeting(x - lengthdir_x(12, objPlayer.dir), y - lengthdir_y(12, objPlayer.dir), objPlayer) {
                    ...
                    if scrIsGang(object_index) my_id.bottom = sprPAttackThroatGang else my_id.bottom = sprPAttackThroat
                    ...
                    global.test = 1
                    if sprite_index = sprEWalkUnarmed or sprite_index = sprEGiveUp or global.maskindex = 11 noweapon = 1
                    else noweapon = 0
                    if noweapon = 0 {
                    ...
                    //Willem throat tear, gets weapon.
                    if global.maskindex = 11 {
                        if scrCurrentWeaponExt(sprite_index) > 0
                        global.gun = scrIndexToWeapon(scrCurrentWeaponExt(sprite_index))
                        else {if instance_exists(objLSD) global.gun = sprPWalkUnarmedLSD else global.gun = sprPWalkUnarmed}
                    }
                }
            }
        }
