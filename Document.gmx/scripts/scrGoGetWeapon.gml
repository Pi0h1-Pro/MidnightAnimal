...
if (weaponfind == 0)
{
	...
	else
	{
		checkreload = 10;
		global.check = 0;
		while (k < instance_number(_WEAPON) && global.check == 0)
		{
	...
	else
	{
		...
		if (place_meeting(x, y, target))
		{
			...
			if (target.image_index == 0)
			{
				...
				my_id.sprite_index = sprEWalkHK417;
			}
			...
			{
				my_id = instance_create(x, y, objEnemyMeleePatrol);
			...
			{
				my_id = instance_create(x, y, objEnemyDodger);
				my_id.sprite_index = sprEDodgerWalk;
			}
			else if (target.image_index == 18)
			{
				my_id = instance_create(x, y, objEnemyMeleeRandom);
				my_id.sprite_index = sprEWalkMachete;
			}
			else if (target.image_index == 26)
			{
				my_id = instance_create(x, y, enemyRandom);
				my_id.sprite_index = sprEWalkAK;
			}
			...
			{
				my_id = instance_create(x, y, objEnemyMeleeRandom);
				my_id.sprite_index = sprEWalkChain;
			}
