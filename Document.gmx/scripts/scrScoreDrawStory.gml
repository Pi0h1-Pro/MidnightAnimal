...
if (global.combo > 0)
{
	...
	if (global.combotime > 0)
	{
		...
		if (global.maskindex == 13)
			global.combotime += 0.75;
		else
			global.combotime += 0.65;
	}
	else
	{
		if (global.combo > 1)
...
if (instance_number(objEnemy) + instance_number(objEnemyBash) + instance_number(objEnemyBashPolice) + instance_number(objEnemyBashGang) == 0)
	update = 1;
if (!instance_exists(objPlayer))
	update = 1;
...
if (instance_exists(objPlayer))
{
	...
	if (objPlayer.sprite_index == sprPWalkAutoShotgun || objPlayer.sprite_index == sprPAttackAutoShotgun)
	{
		showammo = 1;
		maxammo = 20;
	}
	...
	if (objPlayer.sprite_index == sprPWalkSilencedShotgun || objPlayer.sprite_index == sprPAttackSilencedShotgun)
	{
		showammo = 1;
		maxammo = 6;
	}
	if (objPlayer.sprite_index == sprPWalkSuppressedMG || objPlayer.sprite_index == sprPAttackSuppressedMG)
	{
		showammo = 1;
		maxammo = 30;
	}
	...
	if (objPlayer.sprite_index == sprPWalkAK || objPlayer.sprite_index == sprPAttackAK)
	{
		showammo = 1;
		maxammo = 30;
	}
	if (objPlayer.sprite_index == sprPWalkHuntingRifle || objPlayer.sprite_index == sprPAttackHuntingRifle)
	{
		showammo = 1;
		maxammo = 5;
	}
	if (objPlayer.sprite_index == sprPWalkHandgun || objPlayer.sprite_index == sprPAttackHandgun)
	{
		showammo = 1;
		maxammo = 17;
	}
	if (objPlayer.sprite_index == sprPWalkFlamethrower || objPlayer.sprite_index == sprPAttackFlamethrower)
	{
		showammo = 1;
		maxammo = 120;
	}
	if (objPlayer.sprite_index == sprPWalkSniper || objPlayer.sprite_index == sprPAttackSniper)
	{
		showammo = 1;
		maxammo = 13;
	}
	if (objPlayer.sprite_index == sprPWalkNailGun || objPlayer.sprite_index == sprPAttackNailgun)
	{
		showammo = 1;
		maxammo = 30;
	}
...
if (room == rmTrainstationEntrance || room == rmTrainstationDownstairs || room == rmAlley)
{
	if (instance_exists(objPlayer))
	{
		...
		if (global.xbox == 1)
			...
			finishstring = "[SHIFT] TO LOOK FURTHER";
		if (instance_exists(objWeapon))
		{
			if (global.xbox == 1)
				finishstring = "[LEFT TRIGGER] TO TAKE WEAPON";
			else
				finishstring = "[RIGHT MOUSE] TO PICK UP WEAPON";
		}
		if (objPlayer.sprite_index == sprPWalkUnarmed || objPlayer.sprite_index == sprPAttackUnarmed1 || objPlayer.sprite_index == sprPAttackPunch)
			nothing = 1;
		else
		{
		...
		if (instance_exists(objKnockedOut) || instance_exists(objHoboKnockedOut))
		{
			if (global.xbox == 1)
				...
				finishstring = "[SPACE] TO EXECUTE ENEMIES";
		}
	}
	if (!instance_exists(objPlayer) || global.done == 1)
		finish = 0;
}
