if global.xbox = 1 scrXboxPlayerHospitalMove()
else {
    if sprite_index = sprPHospitalHeadache nothing = 1
    else {
        ...
        if keyboard_check_direct(ord(global.upkey)) + keyboard_check_direct(ord(global.downkey)) + keyboard_check_direct(ord(global.leftkey)) + keyboard_check_direct(ord(global.rightkey)) = 0 {
            if myxspeed > 0 myxspeed -= 0.25
            else {
                if myxspeed < -0.25 myxspeed += 0.25
                else myxspeed = 0
            }
            if myyspeed > 0 myyspeed -= 0.25
            else {
                if myyspeed < -0.25 myyspeed += 0.25
                else myyspeed = 0
            }
        }
        ...
        if abs(myxspeed) = 0 and abs(myyspeed) = 0 legindex = 0
            ...
            legindex += add
            with objDizzy {
                if dizziness < 1.25 dizziness += 0.01
                else {
                    with objPlayer {
                        myxspeed = 0 myyspeed = 0 legindex = 0 sound_play(sndTinnitusHospital) image_index = 0 image_speed = 0.15 sprite_index = sprPHospitalHeadache
                    }
                }
            }
