function scrDrawAmmo() {
	...
	if (room == rmTutorial || instance_exists(objMaskMenu))
		nothing = 1;
	else
	{
		...
		{
			scrGetAmmoDraw();
			if (global.ammotype == 2)
				global.magammotype = 5;
			else
				global.magammotype = 4;
			if (global.ammo > 0)
				ammostring = string(global.ammo);
			else
				ammostring = "";
			if (string_length(ammostring) == 1)
				ammostring = "0" + string(global.ammo);
			if (global.ammo < 0)
				ammostring = "NO GUN";
			if (global.done == 1)
				ammostring = "";
			...
			{
				if (global.drawscore + 50 < global.myscore)
					global.drawscore += 50;
				else
					global.drawscore = global.myscore;
			}
			...
			if (scrMovingPlayerExists() && instance_exists(objPlayer))
			{
				global.magammo = objPlayer.mags; //* 32
			}
		
			if (global.ammo + global.magammo > 0 && global.done == 0)
				draw_sprite_ext(sprAmmoSymbols, global.ammotype, 20 + ammox, 232, 1, 1, 0, c_white, 1);
			if (global.magammo > 0 && global.done == 0)
				draw_sprite_ext(sprAmmoSymbols, global.magammotype, 80 + ammox, 232, 1, 1, 0, c_white, 1);
			magammostring = string(global.magammo);
			if (string_length(magammostring) == 1)
				magammostring = "0" + string(global.magammo);
		
			// Draw ammo.
			if (global.ammo > 0 && global.done == 0)
				draw_sprite_ext(sprAmmoSymbols, global.ammotype, 20 + ammox, 232, 1, 1, 0, c_white, 1);
			...
			draw_text_color(34 + ammox, 222, string_hash_to_newline(ammostring), color1, color1, color2, color2, 1);
			...
			draw_text_color(32 + ammox + lengthdir_x(0.5, dir), 220 + lengthdir_x(0.5, dir), string_hash_to_newline(ammostring), color1, color1, color2, color2, 1);
			...
			if (global.magammo > 0 && global.done == 0)
			{
				...
				color1 = merge_color(c_red, c_white, 0.5 + lengthdir_x(0.3, dir));
				color2 = merge_color(c_red, c_fuchsia, 0.5 + lengthdir_x(0.3, dir));
				draw_text_color(94 + ammox, 222, string_hash_to_newline(magammostring), color1, color1, color2, color2, 1);
			
				// Front.
				color1 = merge_color(merge_color(c_aqua, c_white, 0.3 + lengthdir_x(0.2, dir)), c_white, whitealpha);
				color2 = merge_color(merge_color(c_aqua, c_blue, 0.3 - lengthdir_x(0.3, dir)), c_white, whitealpha);
				draw_text_color(92 + ammox + lengthdir_x(0.5, dir), 220 + lengthdir_x(0.5, dir), string_hash_to_newline(magammostring), color1, color1, color2, color2, 1);
			...
			color1 = merge_color(c_red, c_white, 0.5 + lengthdir_x(0.3, dir));
			color2 = merge_color(c_red, c_fuchsia, 0.5 + lengthdir_x(0.3, dir));
			draw_text_color(25 + combox, -5, string_hash_to_newline(string(global.combo) + "x"), color1, color1, color2, color2, 1);
		
			// Front.
			color1 = merge_color(merge_color(c_aqua, c_white, 0.3 + lengthdir_x(0.2, dir)), c_white, whitealpha);
			color2 = merge_color(merge_color(c_aqua, c_blue, 0.3 - lengthdir_x(0.3, dir)), c_white, whitealpha);
			draw_text_color(24 + combox, -6, string_hash_to_newline(string(global.combo) + "x"), color1, color1, color2, color2, 1);
	        ...
	        */
			dir += 4;
			if (global.ammo > -1 || global.done == 1)
			{
				if (ammox < 0)
					ammox += 8;
			}
