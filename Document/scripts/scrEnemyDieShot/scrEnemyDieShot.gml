function scrEnemyDieShot(argument0) {
	if (global.gibfest == 1)
		exit;
	...
	if (bullets > 1)
	{
		...
		if (bullets > 3)
		{
			if (round(random(10)) == 2)
				...
				my_id.sprite_index = choose(sprEDeadShotgunVomit, sprEDeadHeadJerk, sprEShotgunFly, sprEShotgunFly);
			my_id.image_index = 0;
			my_id.image_speed = 0;
		} // Otherwise, just assign a normal death sprite.
		else
			my_id.sprite_index = sprEDeadShotgun;
	...
	else
	{
		...
		if (round(random(5)) == 2 && other.caliber == 1)
		{
