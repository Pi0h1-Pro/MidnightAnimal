	...
	if (checkbutton(0, getid(10)))
	{
		if (ltrig == 0)
		{
			...
			if (reload <= 0)
			{
				...
				with (objEnemy)
				{
					if (scrCollisionLineExt(x, y, objPlayer.x, objPlayer.y, 4) == 0)
						global.exposed = 1;
				}
				lastweapon = scrCurrentWeapon();
				lastammo = ammo;
				...
				if (lastweapon >= 12 && global.test == 0)
					global.test = 1;
				...
				switch (global.test)
				{
					...
					case 2:
						...
						{
							if (lastweapon >= 10)
								my_id = instance_create(x + lengthdir_x(14, dir + left * -20), y + lengthdir_y(12, dir + left * -20), objThrowingWeapon);
							else
								my_id = instance_create(x + lengthdir_x(12, dir), y + lengthdir_y(12, dir), objWeaponThrow);
							sound_play(sndThrow);
							my_id.speed = 8;
