function scrESwatStep() {
		...
		if (point_distance(x, y, target.x, target.y) < 120)
		{
			...
			if (check == 0)
			{
				...
				else
				{
					...
					reload = 5;
					if (place_meeting(x + lengthdir_x(8, angle), y + lengthdir_y(8, angle), objSolid) || place_meeting(x + lengthdir_x(8, angle), y + lengthdir_y(8, angle), objWall) || place_meeting(x + lengthdir_x(8, angle), y + lengthdir_y(8, angle), objSolidCaster))
					{
	...
	if (alert < 2 && alert > 0)
	{
		scrCharge(objPlayer.x, objPlayer.y, 10);
	}
	if (alert == 2)
	{
		scrAim(objPlayer.x, objPlayer.y);
	}
	if (alert == 3)
	{
