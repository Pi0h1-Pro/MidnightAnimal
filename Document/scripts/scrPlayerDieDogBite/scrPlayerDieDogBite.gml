function scrPlayerDieDogBite() {
	with (objPlayer)
	{
		...
		if (object_index == objPlayerBiker)
			my_id.sprite_index = sprNicoleDeadDog;
		else
		...
		if (sprite_index == sprPWalkUnarmed || sprite_index == sprPAttackUnarmed1 || sprite_index == sprPAttackPunch || sprite_index == sprPAttackBash)
			noweapon = 1;
		else
			noweapon = 0;
