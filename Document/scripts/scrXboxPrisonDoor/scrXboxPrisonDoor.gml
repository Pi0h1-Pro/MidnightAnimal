function scrXboxPrisonDoor(argument0) {
	if (gettrigger_r(0) > 100 || gettrigger_l(0) > 100)
	{
		if (solid == 0 || instance_exists(objEnemy) || instance_exists(objKnockedOut))
			exit;
		if (global.locked == 0)
		{
			if (place_meeting(x, y + 4, objPlayer))
			{
				...
				with (objPlayerMouse)
				{
					if (sprite_index == sprPWalkUnarmed || argument0 == sprPAttackUnarmed1 || sprite_index == sprPAttackPunch)
						nothing = 1;
					else
					{
