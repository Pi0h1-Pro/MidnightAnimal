function scrPlayerShootAuto() {
	...
	if (ammo > 0)
	{
		...
		// Battle rifle.
		if (sprite_index == sprPWalkHK417 || sprite_index == sprPTurnHK417)
		{
			sound_ps(sndHK417);
			scrHearPlayer();
			ammo -= 1;
		...
		// The Patriot.
		if (sprite_index == sprPWalkPatriot || sprite_index == sprPTurnPatriot)
		{
			sound_ps(sndPatriot);
			scrHearPlayer();
			reload = 3;
		...
		// Honey Badger.
		if (sprite_index == sprPWalkSuppressedMG && global.automatic == 1)
		{
		...
		// FAMAE.
		if (sprite_index == sprPWalkFAMAE && global.automatic == 1)
		{
			if (reload > 0)
				reload -= 1;
			else
		...
		// Kalashnikov.
		if (sprite_index == sprPWalkAK || sprite_index == sprPTurnAK)
		{
		...
		if (sprite_index == sprPWalkThompson || sprite_index == sprPTurnThompson)
		{
		...
		if (sprite_index == sprPWalkAutoShotgun)
		{
			...
			warp_hud();
			repeat (6)
				projectile(20, objBullet, 18 + random(4), 9, 0.12, 8, sprBullet);
			scrCreateMuzzleFlash(26);
		...
		// MP5.
		if (sprite_index == sprPWalkMP5 || sprite_index == sprPTurnMP5)
		{
			sound_ps(sndMP5);
			scrHearPlayer();
			ammo -= 1;
		...
		if (sprite_index == sprPWalkAJM && global.automatic == 0)
		{
			if (reload > 0)
				reload -= 1;
			else
		...
		if (sprite_index == sprPWalkFlamethrower)
		{
			...
			if (instance_exists(lastflame))
				flame.target = lastflame;
			lastflame = flame;
		...
		if (sprite_index == simoneAim)
		{
			...
			if (ammo == 0)
			{
				...
				projectile(20, objBullet, 16, 8, 0.6, 33, sprBulletRicochet);
			scrCreateMuzzleFlash(36);
			if (ammo == 0)
