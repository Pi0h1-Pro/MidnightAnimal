function hudPierrette() {
	...
	if (pierretteIsShielded(objPlayer.sprite_index)
		|| objPlayer.sprite_index == pierretteWalkSMG
		|| objPlayer.sprite_index == pierretteAttackSMG
		|| objPlayer.sprite_index == pierretteReload
		|| objPlayer.sprite_index == pierretteReloadShield)
		global.maxammo = 32;
	else
		global.maxammo = 2;

	// Draw shield effect.
	draw_sprite_stretched_ext(sprDirtyFilter, 0, 0, 0, __view_get( e__VW.WView, 0 ), __view_get( e__VW.HView, 0 ), c_white, shield_alpha);
	...
	else
	{
		if (scrMovingPlayerExists()) // and pierretteIsShielded(objPlayer.sprite_index)
		{
			...
			draw_text_transformed(24 + gunx, 36, string_hash_to_newline("/// POLICE ASSAULT IN PROGRESS ///"), 0.125, 0.125, 0);
			...
			// Draw ammo.
			draw_sprite_hud(hudPierretteSMG, objPlayer.ammo, __view_get( e__VW.WView, 0 ) - 84, 200, 1, 1, 0);
			draw_sprite_hud(hudPierretteFlashbang, objPlayer.thrown, __view_get( e__VW.WView, 0 ) - 108, 200 - 2, 1, 1, 0);
