function scrPlayerShootSemi() {
	// Check for ammo.
	if (scrIsUnarmed(sprite_index) || scrCurrentWeaponExt(sprite_index) == 80)
		exit;
	...
	if (ammo > 0)
	{
		...
		// SB Geist (crossbow).
		if (sprite_index == sprPWalkCrossbow || sprite_index == sprPTurnCrossbow)
		{
		...
		if (sprite_index == sprPWalkShotgun || sprite_index == sprPTurnShotgun)
		{
			...
			warp_hud();
			repeat (6)
				projectile(20, objBullet, 16 - random(4), 6, 0.3, 15 + random(5), sprShotgunPellet);
		...
		if (sprite_index == sprPWalkKSG)
		{
			...
			warp_hud();
			repeat (6)
				projectile(20, objBullet, 20 - random(4), 3, 0.4, 20 + random(5), sprShotgunPellet);
			scrCreateMuzzleFlash(26);
		...
		if (sprite_index == sprPWalkDoubleBarrel || sprite_index == sprPTurnDoubleBarrel)
		{
			...
			warp_hud();
			repeat (8)
				projectile(20, objBullet, 14, 10, 0.5, 22, sprShotgunPellet);
		...
		if (sprite_index == sprPWalkSawedOff)
		{
			...
			warp_hud();
			repeat (4)
				projectile(20, objBullet, 14, 10, 0.5, 24, sprShotgunPellet);
		...
		if (sprite_index == sprPWalk1887)
		{
			...
			warp_hud();
			repeat (4)
				projectile(20, objBullet, 15 + random(6), 5, 0.5, 30, sprShotgunPellet);
			scrCreateMuzzleFlash(26);
		...
		if (sprite_index == sprPWalkIthaca)
		{
			...
			warp_hud();
			repeat (8)
				projectile(20, objBullet, 15 + random(6), 7, 0.3, 20, sprShotgunPellet);
			scrCreateMuzzleFlash(26);
		...
		// Desert Eagle.
		if (sprite_index == sprPWalkDesertEagle || sprite_index == sprPTurnDesertEagle)
		{
		...
		if (sprite_index == sprPWalkSilencedShotgun)
		{
			...
			warp_hud();
			repeat (6)
				projectile(20, objBullet, 22 - random(3), 6, 0.2, 18, sprShotgunPellet);
		...
		// FAMAE.
		if (sprite_index == sprPWalkFAMAE && global.automatic == 0)
		{
			if (reload > 0)
				reload -= 1;
			else
		...
		// Honey Badger.
		if (sprite_index == sprPWalkSuppressedMG && global.automatic == 0)
		{
		...
		if (sprite_index == simoneWalk)
		{
			...
			if (ammo == 1)
			{
				...
				projectile(20, objBullet, 31, 0, 0.6, 66, sprBullet);
			PROJECTILE.caliber = 1;
			scrCreateMuzzleFlash(36);
			if (ammo == 1)
				stream.image_blend = make_colour_rgb(66, 209, 244);
			else
				stream.image_blend = c_white;
		...
		if (sprite_index == simoneWalkShield)
		{
			...
			if (ammo == 1)
			{
				...
				projectile(20, objBullet, 47, 0, 0.6, 66, sprBullet);
			PROJECTILE.caliber = 1;
			scrCreateMuzzleFlash(36);
			if (ammo == 1)
				stream.image_blend = make_colour_rgb(66, 209, 244);
		...
		if (sprite_index == pierretteWalkShotgun)
		{
			...
			if (place_meeting(x + lengthdir_x(12, dir), y + lengthdir_y(12, dir), objWall)
				|| place_meeting(x + lengthdir_x(12, dir), y + lengthdir_y(12, dir), objDoorV))
			{
			...
			{
				repeat (6)
					projectile(16, objBullet, 12, 0, 0.8, 100, sprBreachingRound);
				smoke = instance_create(x + lengthdir_x(24, dir), y + lengthdir_y(24, dir), objMuzzleSmoke);
				smoke.direction = dir;
				smoke.image_angle = smoke.direction;
			}
