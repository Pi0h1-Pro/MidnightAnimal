function scrBodyBleed() {
	...
	if (speed == 0 && bled == 0)
	{
		...
		if (sprite_index == sprEDeadMachinegun || sprite_index == sprEDeadNail)
			repeat (2 + random(2))
				instance_create(x + lengthdir_x(length, image_angle) - 2 + random(4), y + lengthdir_y(length, image_angle) - 2 + random(4), objBloodPool);
		...
		if (sprite_index == sprEDeadSlashedTop)
			scrBodyPartBleed();
	
		if (sprite_index == sprEDeadBlunt)
		{
			if (image_index == 0 || image_angle == 6)
			{
				repeat (1 + random(3))
				{
		...
		// Head blown off.
		if (sprite_index == sprEDeadShotgun && image_index == 6)
		{
			if (bled == 0)
			{
				if (irandom(10) == 1)
