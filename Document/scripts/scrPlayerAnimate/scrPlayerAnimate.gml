function scrPlayerAnimate() {
	...
	if (global.character == 0)
	{
		if (sprite_index == sprPDrinkSoda)
		{
			...
			if (image_index > 21 && image_index < 22)
			{
				hp += max_hp / 3;
				sound_ps(sndCollectHealth);
				create_health_number(floor(max_hp / 3), 1);
			...
			if (image_index > 25 && image_index < 26)
			{
				hp += max_hp / 3;
				sound_ps(sndCollectHealth);
				create_health_number(floor(max_hp / 3), 1);
	...
	{
		if (sprite_index == pierretteWalkSMG && ready == 1)
		{
	...
	if (global.character == 3)
	{
		...
		if (sprite_index == simoneExecuteShieldSnap)
		{
			...
			if (image_index > 2 && image_index < 4 && dead == 0)
			{
	...
	// Reset.
	if (dead == 1 && scrIsWalking(sprite_index))
		dead = 0;
	if (shield == 1 && scrIsWalking(sprite_index))
		shield = 0;
