function scrBerserk() {
	if (instance_exists(objPlayer))
	{
		...
		if (kills > 0)
		{
			if (killamount < kills * 0.5)
				killamount = kills * 0.5;
			if (killamount > 1)
				killamount = 1;
		}
		...
		if (kills >= 5)
		{
			objPlayer.berserk = 1;
			draw_set_blend_mode(bm_subtract);
			draw_circle_color(__view_get( e__VW.WView, 0 ) / 2, __view_get( e__VW.HView, 0 ) / 2, 480 + random(24), merge_color(c_black, merge_color(c_teal, c_aqua, 0.6 + lengthdir_x(0.4, global.dir * 4)), bamount), merge_color(c_black, c_white, bamount), 0);
			draw_circle_color(__view_get( e__VW.WView, 0 ) / 2, __view_get( e__VW.HView, 0 ) / 2, 480 + random(24), merge_color(c_black, merge_color(c_teal, c_aqua, 0.6 + lengthdir_x(0.4, global.dir * 4)), bamount), merge_color(c_black, c_white, bamount), 0);
			draw_set_blend_mode(bm_normal);
