function L3S_shadow_update_soft(argument0) {
	...
	if (LS_imax > 0)
	{
		...
		for (LS_i2 = 0; LS_i2 < LS_imax; LS_i2 += 1)
		{
			...
			if ((LS_shadow[LS_i2].LS_z) != (LS_shadow[LS_i1].LS_z) || (LS_i2 == LS_imax - 1))
			{
				...
				{
					LS_above = LS_shadow[LS_i1].LS_z < LS_z;
				}
				for (i = LS_i1; i < LS_i2; i += 1)
				{
					with (LS_shadow[i])
					{
						...
						{
							draw_sprite_ext(LS_mat_map, LS_bump_map1, x, y, image_xscale, image_yscale, image_angle, c_white * other.LS_above, image_alpha);
						}
					}
				}
				LS_i1 = LS_i2;
