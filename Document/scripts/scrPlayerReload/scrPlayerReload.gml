function scrPlayerReload() {
	if (scrIsReloading(sprite_index))
	{
		...
		if (sprite_index == sprPReloadSuppressedMG)
		{
			if (image_index < sprite_get_number(sprite_index) - 1)
			{
				...
				if (image_index >= 7 && image_index - global.reloadspeed < 7)
				{
		...
		if (sprite_index == sprPReloadHK417)
		{
			...
			{
				image_index += global.reloadspeed;
				if (image_index >= 3 && image_index - global.reloadspeed < 3)
				{
					sound_ps(sndEjectClipRifle);
					my_id = instance_create(x + lengthdir_x(5, dir - 5 * left), y + lengthdir_y(5, dir - 5 * left), objGunClip);
					my_id.image_index = 3;
		...
		if (sprite_index == sprPReloadDoubleBarrel)
		{
			if (image_index < sprite_get_number(sprite_index) - 1)
			{
				...
				if (image_index >= 11 && image_index - global.reloadspeed * 1.5 < 11)
					sound_ps(sndInsertShell);
				if (image_index >= 16 && image_index - global.reloadspeed * 1.5 < 16)
					sound_ps(sndInsertShell);
			}
			else
			{
		...
		if (sprite_index == sprPReloadPistol)
		{
			...
			{
				image_index += global.reloadspeed;
				if (image_index >= 5 && image_index - global.reloadspeed < 5)
				{
				...
				if (image_index >= 10 && image_index - global.reloadspeed < 10)
					sound_ps(sndInsertClip);
			}
			else
			{
				ammo += (global.maxammo - ammo);
		...
		if (sprite_index == simoneReload)
		{
			...
			{
				image_index += global.reloadspeed;
				if (image_index >= 1 && image_index - global.reloadspeed < 1)
				{
				...
				if (image_index >= 15 && image_index - global.reloadspeed < 15)
					sound_ps(sndRevolverShellsIn);
				if (image_index >= 17 && image_index - global.reloadspeed < 17)
					sound_ps(sndRevolverClose);
			}
			else
			{
				ammo += (global.maxammo - ammo);
		...
		if (sprite_index == sprPReloadKnifePistol)
		{
			...
			{
				image_index += global.reloadspeed;
				if (image_index >= 2 && image_index - global.reloadspeed < 5)
				{
					sound_ps(sndEjectClip);
					my_id = instance_create(x + lengthdir_x(5, dir - 5 * left), y + lengthdir_y(5, dir - 5 * left), objGunClip);
					my_id.image_index = 3;
		...
		if (sprite_index == sprPReloadSniper)
		{
			...
			{
				image_index += 0.2;
				if (image_index >= 7 && image_index - 0.2 < 7)
				{
		...
		if (sprite_index == sprPReloadDisruptor)
		{
			...
			{
				image_index += global.reloadspeed;
				if (image_index >= 8 && image_index - global.reloadspeed < 8)
				{
	...
	if (ammo == 0 || keyboard_check_pressed(ord("R")))
		scrGetPlayerReload();
