		...
		if (scrMovingPlayerExists())
		{
			...
			if (global.combo > 0)
			{
				...
				if (global.combo mod 5 == 0)
				{
					...
					draw_text_hud(450 + combox + ammox, 200, string(global.combo) + "x", 0.5, 0.5, 0);
			}
			...
			if (scrCurrentWeaponExt(objPlayer.sprite_index) == 64)
				draw_text_hud(54 + ammox, 205, "Inf.", 0.25, 0.25, 0); // The Patriot.
			else
				draw_text_hud(54 + ammox, 205, ammostring, 0.25, 0.25, 0);
		
			// Draw score.
			if (global.myscore > 0)
			{
				draw_set_halign(fa_right);
				draw_text_hud(__view_get( e__VW.WView, 0 ) - 24 + scorex, 14, string(global.drawscore) + " pts", 0.15, 0.15, 0);
				draw_set_alpha(1);
