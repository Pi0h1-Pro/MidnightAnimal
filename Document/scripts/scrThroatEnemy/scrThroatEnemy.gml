function scrThroatEnemy() {
	if (sprite_index == sprPWalkBag || sprite_index == sprPAttackBag || global.maskindex == 2)
		nothing = 1;
	else
	{
		global.test = 0;
		if (place_meeting(x + lengthdir_x(12, dir), y + lengthdir_y(12, dir), objEnemy))
		{
			with (objEnemy)
			{
				if (object_index == objDogPatrol
					|| object_index == objEnemyFatLSD
					|| object_index == objEnemyFat
					|| object_index == objEnemyFatStatic
					|| object_index == objEnemyDodger
					|| object_index == objCommando
					|| scrIsPolice(object_index)
				...
				else
				{
					if (place_meeting(x - lengthdir_x(12, objPlayer.dir), y - lengthdir_y(12, objPlayer.dir), objPlayer))
					{
						...
						if (scrIsGang(object_index))
							my_id.bottom = sprPAttackThroatGang;
						else
							my_id.bottom = sprPAttackThroat;
						...
						if (global.maskindex == 11)
						{
							...
							else
							{
								if (instance_exists(objLSD))
									global.gun = sprPWalkUnarmedLSD;
								else
									global.gun = sprPWalkUnarmed;
							}
						}
					}
				}
			...
			if (global.test == 1)
			{
				if (sprite_index == sprPWalkUnarmed || sprite_index == sprPWalkUnarmedLSD)
					nothing = 1;
				else
				{
