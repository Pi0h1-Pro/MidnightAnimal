function scrPlayerPickUp(argument0) {
	// Pick up letter.
	...
	global.test = 0;
	if (sprite_index == sprPWalkUnarmed || argument0 == sprPAttackUnarmed1 || sprite_index == sprPWalkUnarmedLSD || sprite_index == sprPAttackPunch)
		noweapon = 1;
	else
		noweapon = 0;
	with (objWeapon)
	{
		if (global.test == 0)
		{
			if (point_distance(x, y, objPlayer.x, objPlayer.y) < 32 && global.maskindex != 2)
			{
				if (ammo > 0)
				{
					global.etest = 0;
	...
	if (noweapon == 1 || global.maskindex == 15 || global.maskindex == 7)
		global.test = 4;
	else
	{
		if (lastweapon > 49)
