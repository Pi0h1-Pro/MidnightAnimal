function draw_interact_text() {
	...
	if (string_length(text_interact) > 0)
	{
		if (global.character == 0)
		{
			...
			draw_rectangle(__view_get( e__VW.WView, 0 ) / 2 - string_width(string_hash_to_newline(text_interact)) * 0.1,
				222 - 8, // view_hview[0] * 0.9 - 6,
			__view_get( e__VW.WView, 0 ) / 2 + string_width(string_hash_to_newline(text_interact)) * 0.1,
				222 + 8, // view_hview[0] * 0.9 + 6,
			0);
			draw_set_alpha(1);
		
			draw_set_color(global.albedo);
			draw_text_transformed(__view_get( e__VW.WView, 0 ) / 2, 222, string_hash_to_newline(text_interact), 0.125, 0.125, 0);
		}
		...
		if (global.character == 2)
		{
			...
			draw_set_color(global.albedo);
			draw_text_transformed(24 + gunx, 172, string_hash_to_newline(text_interact), 0.125, 0.125, 0);
		}
	}
