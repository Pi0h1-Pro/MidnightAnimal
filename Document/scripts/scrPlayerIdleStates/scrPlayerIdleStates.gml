function scrPlayerIdleStates() {
	if (sprite_index == sprJohnSmokeCigarette)
	{
		...
		if (image_index >= 13 && image_index - 0.2 < 13)
			sound_ps(sndLighter);
		if (image_index >= 46 && image_index - 0.2 < 46)
			sound_ps(sndInhale);
	
		if (image_index > 71 && exhaled == 0)
		{
			// Create smoke cloud.
			sound_ps(sndExhale);
	...
	if (instance_exists(mnPhone))
	{
		...
		if (!mnPhone.active && sprite_index == sprJohnUsePhone)
		{
