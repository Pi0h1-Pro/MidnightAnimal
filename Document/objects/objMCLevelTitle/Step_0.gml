...
draw_set_color(merge_color(c_white, c_fuchsia, 0.5 + lengthdir_y(0.5, pointx * 2)));
draw_text(x + 1, y - 35, string_hash_to_newline(global.pretitle));
draw_set_color(merge_color(c_white, c_fuchsia, 0.5 + lengthdir_x(0.5, pointx * 2)));
draw_text(x, y - 36, string_hash_to_newline(global.pretitle));
draw_set_font(fntBikerTitle);
i = 0;
pointx += 3;
...
repeat (string_length(global.title))
{
	...
	if (on[i] == 1)
	{
		...
		{
			if (dist > 1)
				draw_set_color(merge_color(c_fuchsia, c_yellow, ii * (1 / 15)));
			else
				draw_set_color(merge_color(merge_color(c_orange, c_purple, random(1)), darkcolor, amount[i]));
