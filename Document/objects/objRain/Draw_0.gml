...
if (irandom(900) == 5)
{
	if (!audio_is_playing(sndThunder))
		audio_play_sound(sndThunder, 5, 0);
}
...
// Drop count.
if (mouse_wheel_up())
	scrRainSetDropCount(rain_drops + 10);
else if (mouse_wheel_down())
	scrRainSetDropCount(rain_drops - 10);

// Drop size.
if (keyboard_check(vk_up))
	rain_size = min(4, rain_size + 0.1);
if (keyboard_check(vk_down))
	rain_size = max(1, rain_size - 0.1);

// Drop length.
if (keyboard_check(vk_right))
	rain_length = min(2, rain_length + 0.05);
if (keyboard_check(vk_left))
	rain_length = max(0.1, rain_length - 0.05);
