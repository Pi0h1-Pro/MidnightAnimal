...
if (state == 0)
{
	if (objPlayer.y < 476)
	{
		with (objPlayerMouse)
		{
			...
			if (sprite_index == sprPWalkUnarmed || sprite_index == sprPAttackPunch || sprite_index == sprPAttackUnarmed1)
				nothing = 1;
			else
			{
...
if (state == 4)
{
	if (!instance_exists(objPhoneConversation))
	{
		...
		{
			if (image_index < 9 && global.maskindex == 2 || global.maskindex == 0)
				image_index += 0.3;
			else
			{
				image_index = 0;
			...
			if (image_index < 9 && sprite_index == sprBoss4GetGuns)
				image_index += 0.2;
			else
			{
				image_index = 0;
...
if (state == 5 || (global.maskindex == 2 || global.maskindex == 0 && point_distance(x, y, objPlayer.x, objPlayer.y) < 64 && state <= 5 && !instance_exists(objBodyguard) && !instance_exists(objBodyguardCrawl)))
{
