...
else
{
	...
	{
		if (active == 1 || (!instance_exists(objPhone) && !instance_exists(objBoss4) && !instance_exists(objBossPhone) && !instance_exists(objTrain)) || instance_exists(objBodyguard) && !instance_exists(objPicture))
		{
			vdist = point_distance(x, y, room_width / 2 - (1 - valid) * 88, room_height / 2);
			vdir = point_direction(room_width / 2 - (1 - valid) * 88, room_height / 2, x, y);
			viewspeed = point_distance(__view_get( e__VW.XView, 0 ) + __view_get( e__VW.WView, 0 ) / 2 - (1 - valid) * 88, __view_get( e__VW.YView, 0 ) + __view_get( e__VW.HView, 0 ) / 2, x + lengthdir_x(vdist * 0.2, vdir - 180), y + lengthdir_y(vdist * 0.2, vdir - 180)) * 0.1;
...
if (sprite_index == sprPAttackPunch || sprite_index == sprPAttackUnarmed1)
{
	...
	with (objEnemy)
	{
		if (object_index == objDogPatrol)
			...
			valid = 1;
		if (place_meeting(x, y, objPlayer) && valid == 1)
		{
			mask_index = sprWeaponMask;
			if (scrCollisionLineExt(x, y, objPlayer.lastx, objPlayer.lasty, 4))
			{
