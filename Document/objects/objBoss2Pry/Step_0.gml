if (objPlayer.sprite_index == sprPAttackClub || (global.maskindex == 2 && (objPlayer.sprite_index == sprPAttackPunch || objPlayer.sprite_index == sprPAttackUnarmed1)))
{
	global.test = 0;
	with (objPlayer)
	{
	...
	if (global.test == 1)
	{
		...
		with (my_id)
		{
			...
			repeat (3)
			{
				my_id = instance_create(x - 3 + random(6) + lengthdir_x(16, pdir), y - 3 + random(6) + lengthdir_y(16, pdir), objBigBlood);
