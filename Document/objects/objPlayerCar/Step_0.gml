...
if (done == 2 && global.done == 0)
{
	...
	{
		if (doorindex > 0)
			sound_ps(sndCarClose);
		doorindex = 0;
	}
}

if (scrMovingPlayerExists() && global.done == 1)
{
	if (point_distance(x + lengthdir_x(6, image_angle), y, objPlayer.x, objPlayer.y) < 64 && objPlayer.visible == 1)
	{
		if (enter == 0)
		{
			if (doorindex < 6)
	...
	if (place_meeting(x, y, objPlayer) || enter == 1)
	{
		enter = 1;
	...
	if (instance_exists(objPlayer) && global.done == 0)
	{
		mask_index = sprite_index;
		exit;
	}
