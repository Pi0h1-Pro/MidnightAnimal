...
if (instance_exists(objPlayer))
{
	...
	switch (int_state)
	{
		...
		case 1:
			...
			if (b_aim)
			{
				// If I am aiming, switch the sprite index to the player aiming
			...
			if (point_in_triangle(objPlayer.x, objPlayer.y, x, y, x + lengthdir_x(500, image_angle - 30), y + lengthdir_y(500, image_angle - 30), x + lengthdir_x(500, image_angle + 30), y + lengthdir_y(500, image_angle + 30))
				&& !collision_line(x, y, objPlayer.x, objPlayer.y, objWall, false, true))
			{
				// Make sure the player is in my vision cone and a wall is not in the way
				...
				for (b = 0; b <= ds_priority_size(pri_coverObj) - 1; b++)
				{
					// We are going to find cover somewhere around the cover object
					...
					for (d = 0; d <= 3; d++)
					{
						...
						&& !collision_line(x1, y1, objPlayer.x, objPlayer.y, objWall, false, true))
						{
							// Does a line between this location and the player not intersect a wall?
		...
		case 2:
			...
			else
			{
				...
				else
				{
					if (alarm[0] == -1)
					{
						// Made it to the end of the path
				...
				if (int_ammo > 0)
				{
					// Make sure I have ammo too
					if (alarm[1] == -1 && alarm[2] == -1 && !b_crouch)
					{
						// Make sure I am not crouching when I fire (also make sure alarm[2] is -1) can't have him shooting the moment he sees the player
						var xx, yy;
						...
						with (instance_create(x + lengthdir_x(5, image_angle), y + lengthdir_y(5, image_angle), objShell))
						{
							// Create cartridge
					...
					if (b_crouch)
					{
						// If I am crouching, there is no recoil
						image_index = 0;
					}
					else
					{
						...
						if (alarm[1] mod 3 == 0)
						{
							// If alarm[0] is a multiple of 3, switch the image_index from 0-1 or 0-1
