...
if (state == 4)
{
	...
	{
		wait -= 1;
		scrSongVolume(wait * (1 / 60));
		if (wait == 0)
		{
			scrSongVolume(1);
			scrPlaySong(working_directory + "\\ElectricDreams.mp3");
		}
	}
	else
	{
...
if (state == 5)
{
	...
	else
	{
		with (objPlayer)
		{
			...
			if (global.throw == 0 && image_index > 16)
			{
...
if (state == 6)
{
	...
	else
	{
		...
		with (objPlayer)
		{
			...
			if (image_index > 122 && global.picture == 0)
			{
				global.picture = 1;
