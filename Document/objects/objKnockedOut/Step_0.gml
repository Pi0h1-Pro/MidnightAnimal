...
if (scrIsLeaning(sprite_index))
{
	if (instance_exists(objPlayer))
	{
		...
		if (scrIsBlunt(objPlayer.sprite_index))
		{
			if (place_meeting(x, y, objPlayer))
			{
				...
				if (sprite_index == sprEGetUpLean)
					my_id.sprite_index = sprEDeadLeanMelee;
				my_id.image_angle = angle;
				scrMeleeBlood(x, y);
				if (objPlayer.sprite_index == sprPBatKill)
					sound_ps(sndBatHit);
				sound_ps(sndPunch);
			...
			|| objPlayer.sprite_index == sprPAttackAxe
			|| objPlayer.sprite_index == sprPAttackChainsaw
			|| objPlayer.sprite_index == sprPAttackKatanaH1
			|| objPlayer.sprite_index == sprPAttackKatanaH2
			|| objPlayer.sprite_index == sprPAttackLongSword1
			|| objPlayer.sprite_index == sprPAttackLongSword2)
		{
			if (place_meeting(x, y, objPlayer))
			{
				my_id = instance_create(x, y, objDeadBody);
			...
			|| objPlayer.sprite_index == sprPAttackKnifeSlash1
			|| objPlayer.sprite_index == sprPAttackKnifeSlash2
			|| objPlayer.sprite_index == sprPAttackKitchenKnife
			|| objPlayer.sprite_index == sprPAttackBottleBroken)
		{
			if (place_meeting(x, y, objPlayer))
			{
				my_id = instance_create(x, y, objDeadBody);
		...
		else if (scrIsUnarmed(objPlayer.sprite_index) && scrIsSwinging(objPlayer.sprite_index) && global.character == 1)
		{
			if (place_meeting(x, y, objPlayer))
			{
				my_id = instance_create(x, y, objDeadBody);
				if (sprite_index == sprEGetUpLean)
					my_id.sprite_index = sprEHurtLeanMelee;
				my_id.image_angle = angle;
				sound_ps(sndDoorHit);
