...
if (!scrMovingPlayerExists())
	exit;
if (locked == 1 || abs(swingspeed) > 3.5)
	nothing = 1;
else
{
	global.my_id = id;
		...
		if (place_meeting(x, y, global.my_id))
		{
			...
			with (global.my_id)
			{
				...
				if (abs(swingspeed) > 3.5)
					exit;
				if (abs(swingspeed) < 2)
					sound_ps(choose(sndDoorOpen1, sndDoorOpen2));
				if (x > target.x && target.y < y)
				{
					swingspeed = -16;
					exit;
				}
				...
				if (target.y < y + lengthdir_y(32, image_angle))
					swingspeed = -16;
				else
					swingspeed = 16;
			}
	...
	if (point_distance(x, y, objPlayer.x, objPlayer.y) < 32)
	{
		if (keyboard_check_pressed(global.interactkey))
		{
			...
			if (abs(swingspeed) < 2)
				sound_ps(choose(sndDoorOpen1, sndDoorOpen2));
			if (x > objPlayer.x && objPlayer.y < y)
			{
				swingspeed = -16;
				exit;
			}
			...
			if (objPlayer.y < y + lengthdir_y(32, image_angle))
				swingspeed = -16;
			else
				swingspeed = 16;
		}
	...
	if (place_meeting(x - lengthdir_x(12, objPlayer.dir), y - lengthdir_y(12, objPlayer.dir), objPlayer))
	{
			...
			|| objPlayer.sprite_index == pierretteBashShield)
		{
			...
			if (objPlayer.y > y)
				...
				my_id.image_angle = 270 + random_range(-20, 20);
