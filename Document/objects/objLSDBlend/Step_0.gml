...
if (abs(scale) == 0)
	draw_surface_ext(surf2, -0.5 + random(1), -0.5 + random(1), 1, 1, 0, c_white, 0.999);
else
	draw_surface_ext(surf2, screenx - __view_get( e__VW.XView, 0 ) + scale * 0.5 + (-0.5 + random(1)) * (1 / 3), screeny - __view_get( e__VW.YView, 0 ) + scale * 0.5 + (-0.5 + random(1)) * (1 / 3), (1 + abs((scale / (surface_get_width(surf2) * (1 / 3))) * 2)) * (1 / 3), (1 + abs((scale / (surface_get_height(surf2) * (1 / 3))) * 2)) * (1 / 3), 0, merge_color(c_white, make_color_hsv(h, 255, 255), 0.1), 0.999);
...
with (objSurfacer)
	draw_surface_ext(surf, -__view_get( e__VW.XView, 0 ), -__view_get( e__VW.YView, 0 ), 0.5, 0.5, 0, c_white, 1);
with (objLSDBushVines)
...
with (objPlayerMouse)
{
	...
	if (sprite_index == sprPWalkShotgunLSD)
		sprite = sprPShotgunLSD;
	if (sprite_index == sprPAttackShotgunLSD)
		sprite = sprPShotgunAttackLSD;
	if (sprite_index == sprPWalkAKLSD)
		sprite = sprPAKLSD;
	if (sprite_index == sprPAttackAKLSD)
		sprite = sprPAKAttackLSD;
	if (sprite_index == sprPWalkM16LSD)
		sprite = sprPM16LSD;
	if (sprite_index == sprPAttackM16LSD)
		sprite = sprPM16AttackLSD;
	if (sprite_index == sprPWalkSilencerLSD)
		sprite = sprPSilencerLSD;
	if (sprite_index == sprPAttackSilencerLSD)
		sprite = sprPSilencerAttackLSD;
...
with (objEnemyFatLSD)
{
	...
	if (speed > 0 || path_index >= 0)
		draw_sprite_ext(sprEFatLegs, image_index * 2, x - __view_get( e__VW.XView, 0 ), y - __view_get( e__VW.YView, 0 ), image_xscale, image_yscale, direction, image_blend, 1);
