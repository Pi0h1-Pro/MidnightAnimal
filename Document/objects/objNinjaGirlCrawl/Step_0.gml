if (sprite_index == sprBodyguardTurn)
{
	if (image_index < 4.25)
		...
		sprite_index = sprBodyguardCrawl;
	exit;
}
...
else
{
	if (point_distance(x, y, objPlayer.x, objPlayer.y) < 12 && objPlayer.object_index == objPlayerMouse)
	{
		with (objPlayer)
		{
			if (sprite_index == sprPWalkUnarmed || sprite_index == sprPWalkBag || sprite_index == sprPAttackPunch || sprite_index == sprPAttackUnarmed1)
				nothing = 1;
			else
			{
		...
		repeat (3)
		{
			...
			my_id.image_index = 49; // 52
		}
		my_id = instance_create(x, y, objPNinjaGirlBash);
