...
with (all)
{
	if (sprite_index > 0)
	{
		...
		else
		{
			...
			draw_sprite_ext(sprite_index, image_index, global.x + lengthdir_x(point_distance(x, y, global.x, global.y), global.dir + drawdir), global.y + lengthdir_y(point_distance(x, y, global.x, global.y), global.dir + drawdir), image_xscale, image_yscale, global.dir + image_angle, image_blend, image_alpha);
		}
...
if (fade == 1)
{
	...
	else
	{
		if (restart == 0)
		{
			scrStopSong();
			scrSongVolume(1);
			scrSongVolume(working_directory + "\\WeDontTalkMuchAnymore.mp3");
