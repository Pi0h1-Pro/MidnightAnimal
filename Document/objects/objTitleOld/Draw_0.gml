...
__view_set( e__VW.Angle, 0, lengthdir_y(1, dir * 1.6) );
	...
	{
		{
			if (select == 0)
			{
				...
				sxeasy_stop();
				sxeasy_setVolume(0); // sxeasy_play(working_directory+"\ItsSafeNow.mp3")
				sxeasy_play(working_directory + "\\Static.mp3");
				room = nextlevel;
			}
			if (select == 1 && level == 1)
				room = rmControls; // sxeasy_play(working_directory+choose("\Knock.mp3",       "\InnerAnimal.mp3",       "\Hotline.mp3",       "\NearDark.mp3",       "\Crystals.mp3")) room      =      rmChapter
			if (select == 1 && level == 0)
				room = rmLevelSelect;
			if (select == 3)
				room = rmBandCredits;
