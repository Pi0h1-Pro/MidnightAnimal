...
if checkbutton(0, getid(10)) {
    if ltrig = 0 {
        ...
        if reload <= 0 {
            ...
            global.drop = 1
            with objEnemy {
                if scrCollisionLineExt(x, y, objPlayer.x, objPlayer.y, 4, objSolid, objSolidCaster, objDoorV, objWall, objWallSoftH) = 0 global.exposed = 1
            }
            lastweapon = scrCurrentWeapon()
            lastammo = ammo
            ...
            global.test = 0
            if sprite_index = sprPWalkUnarmed noweapon = 1
            else noweapon = 0
            with objWeapon {
                if global.test = 0 {
                    if point_distance(x, y, objPlayer.x, objPlayer.y) < 32 {
                        if ammo > 0 {
                            global.etest = 0
                            with objEnemy {
                                if sprite_index = sprEWalkUnarmed nothing = 1
                                else {
                        ...
                        if objPlayer.sprite_index = sprPWalkUnarmed nothing = 1
                        else {
            ...
            switch global.test {
                ...
                case 2:
                    if noweapon = 0 {
                        if lastweapon >= 10 my_id = instance_create(x + lengthdir_x(14, dir + left * -20), y + lengthdir_y(12, dir + left * -20), objThrowingWeapon)
                        else my_id = instance_create(x + lengthdir_x(12, dir), y + lengthdir_y(12, dir), objWeaponThrow)
                        sound_play(sndThrow)
                        my_id.speed = 8
                        my_id.direction = dir - 5 + random(10)
                        my_id.ammo = lastammo
