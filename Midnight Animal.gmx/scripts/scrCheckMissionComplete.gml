if instance_exists(objPlayer) {
    if room = rmApartment1Upstairs {
        if !instance_exists(objEnemy) {
            if !instance_exists(objKnockedOut) and!instance_exists(objEHideM16) {
                {
                    if global.done = 0 and instance_number(objEBullet) = 0 {
                        instance_create(x, y, objLevelComplete) global.done = 1
                    }
                }
            }
        }
    ...
    if room = rmHotelSuite {
        if instance_number(objEnemy) + instance_number(objKnockedOut) + instance_number(objEHideM16) = 0 {
           // if objFatmanShit.sprite_index = sprFatmanShitDie or objFatmanShit.sprite_index = sprFatmanShitDecapitated or global.maskindex = 2 
           {
                if global.done = 0 and instance_number(objEBullet) = 0 {
                    instance_create(x, y, objLevelComplete) global.done = 1
                }
    ...
    if room = rmMansionMainHall {
        if !instance_exists(objBoss) {
            ...
            with objBossPhone {
                if on = 0 {
                    on = 1 sound_loop(sndPhoneCall)
                }
            }
            if !instance_exists(objBossgun) and objBossPhone.talked = 1 {
                {
                    instance_create(x, y, objLevelComplete) global.done = 1
                }
            }
    ...
    if room = rmBuildingFloor1 {
        if instance_number(objEnemy) + instance_number(objKnockedOut) + instance_number(objEHideM16) = 0 {
            ...
            if global.done2 = 0 {
                with objTeleporter {
                    if nextroom = room_previous(room) sprite = sprArrowGray
                }
            } else {
                with objTeleporter {
                    if nextroom = room_previous(room) sprite = sprGoArrow
                }
            }
        }
        if global.done1 + global.done2 + global.done3 + global.done4 = 4 {
            if global.done = 0 and instance_number(objEBullet) = 0global.done = 1
        }
    }
